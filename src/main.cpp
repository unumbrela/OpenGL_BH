...
// Include irrKlang header for audio
#include <irrKlang.h>
...
int main(int, char**) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Ensure texBlackhole is not redefined
        // Remove the following line to prevent redefinition
        ...
        static GLuint texDownsampled[MAX_BLOOM_ITER] = { 0 };
        static GLuint texUpsampled[MAX_BLOOM_ITER] = { 0 };
    ...
    // Cleanup irrKlang sound engine
    if (soundEngine) {
        soundEngine->drop(); // Releases the sound engine
    }
